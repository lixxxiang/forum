## BUG FIX
* [x] 添加地图服务后，图层管理面板未刷新，需手动刷新
* [x] 采集列表和本地储存图斑无法删除
* [x] 图斑信息滑动区域触发有误
* [x] 点击遮罩部分无法关闭弹窗
* [x] 退出登录无弹窗提示
* [x] 本地储存图斑列表删除：左滑删除比较符合iOS逻辑
* [ ] 列表中应增加筛选功能，如基于类型和区域的条件查询，增加采集条数计数功能，以便在较大区域使用过程中对采集情况整体把握较好
* [ ] 应具打开kml文件、图斑导航功能：野外信息采集会提前规划路线和点位，借鉴本部门其他APP离线/在线导航，指引信息采集或者核查人通过导航功能到达预先规划目的地
* [x] 提交之后，本地保存的可以重复提交
* [x] 使用聚类方式解决大量Markers导致的地图卡顿问题
## WHAT'S NEW
* [x] 使用第三方Sheet组件，使上拉抽屉更容易使用、性能更高、更稳定、更可定制并且具有更多功能
* [x] 在采集方式中新增地块采集功能等基础地理工具功能
* [x] 使用聚类方式解决大量Markers导致的地图卡顿问题
* [x] 使用链式操作优化部分网络请求逻辑，相对于响应式请求，调用时机及获取response状态更稳定
* [x] 简化图斑状态，限制部分场景的图斑状态修改功能，如已提交的图斑关闭再次提交功能入口，已保存本地的图斑关闭再次保存入口
* [x] 图层管理功能优化，通过本地数据库保存图层显示/隐藏状态及图层的覆盖显示顺序
* [x] 图层添加功能优化，对用户输入的图层链接进行可能发生的错误修正，如WMTS图层的y轴修正，WMS图层冗余参数的删除等，最大程度确保图层正确添加
* [x] 使用WMS图层类型加载图斑，并支持特定图斑的信息修改
* [x] 离线地图功能的优化，Android端使用ExpansionTile，iOS端使用GroupedListView
* [x] 新增手动绘制离线地图功能，至此离线地图的下载包括根据WMS图斑自动下载和手动绘制区域两种方式
* [ ] 测量功能，包括多点位的距离测量，区域面积的测量功能
* [ ] 除基础地理工具外，实现作物种植可视化管理，作物生长高频次监测，气象灾害提前预知等专业功能  
